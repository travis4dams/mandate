...
## Macro dynamics
- **SPEC-SIM-5** `[testable]` A pure `applyMacroDynamics(state, params): GameState` in `src/engine/dynamics.ts` evolves `inflation` and `unemployment` each month using a simplified Phillips-curve / IS-curve model. `inflation` is updated via: `persistence * inflation + (1-persistence) * expectations_anchor - phillips_slope * (unemployment - natural_rate)`. `unemployment` is updated via: `unemployment + rate_sensitivity * (policy_rate - neutral_rate)`. Both are clamped (inflation >= 0, unemployment in [0,1]). Params (`phillips_slope`, `unemployment_natural_rate`, `rate_sensitivity`, `neutral_rate`, `inflation_persistence`) live in `content/engine/dynamics.json` (schema-governed). `applyMacroDynamics` is called once per month inside `Session.advance()` after `applyMonthlySpiral`.
