...
const params: Record<string, FogParams> = loadValidatedFile<Record<string, FogParams>>(SCHEMA_PATH, FILE_PATH);
...
export function observe(
  ...
  const fogEntry = params[seriesId];
  if (fogEntry === undefined) {
    throw new Error(
      `observe: unknown seriesId "${seriesId}" — add it to content/engine/fog.json`
    );
  }
