...
function sampleNormal(rng: () => number, mean: number, std: number): number {
  // Avoid log(0) by rejecting u1 === 0 (probability 0 for any finite uniform).
  // In practice mulberry32 never returns exactly 0, but this guard is defensive.
  let u1 = rng();
  if (u1 === 0) u1 = rng(); // retry once; still seeded, still deterministic.
  const u2 = rng();
